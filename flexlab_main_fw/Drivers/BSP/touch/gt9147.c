...
uint8_t GT9147_Scan(uint8_t mode)
{
	...
	if((mode&0XF)&&((mode&0XF)<6))
	{
		...
		tempsta=sta;			//保存当前的tp_dev.sta值
		sta=(~temp)|TP_PRES_DOWN|TP_CATH_PRES; 
		t_x[4]=t_x[0];	//保存触点0的数据
		t_y[4]=t_y[0];
		for(i=0;i<5;i++)
		{
			if(sta&(1<<i))	//触摸有效?
			{
				...
				if(1)//横屏
				{
					t_y[i]=((uint16_t)buf[3]<<8)+buf[2];
					t_x[i]=((uint16_t)buf[1]<<8)+buf[0];
				}
				else
				{
					t_x[i]=1024-(((uint16_t)buf[1]<<8)+buf[0]);
					t_y[i]=((uint16_t)buf[3]<<8)+buf[2];
				} 
			}			
		} 
...
uint8_t gt_get_touch(uint16_t *x,uint16_t *y)
{
	uint8_t press;
	press=GT9147_Scan(0);
	*x=t_x[0];
